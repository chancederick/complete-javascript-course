...
const checkDogs = function (dogsJulia, dogsKate) {
  const correctDogJulia = dogsJulia.slice(1, -2);
  const allDogs = [...correctDogJulia, ...dogsKate];
  allDogs.forEach(function (move, i) {
    if (move >= 3) {
      console.log(`Dog number ${i + 1} is an adult, and is ${move} years old`);
    } else {
...
const calcDogAge = function (arr) {
  const humanAge = arr.map(mov => (mov <= 2 ? 2 * mov : 16 + mov * 4));
  const filterHumanAge = humanAge.filter(age => age >= 18);
  
  return filterHumanAge.reduce((acc, cur) => acc + cur) / filterHumanAge.length;
};

//Coding Challenge #3
const calcDogAge2 = arr =>
arr
.map(mov => (mov <= 2 ? 2 * mov : 16 + mov * 4))
.filter(age => age >= 18)
.reduce((acc, cur, i, arr) => acc + cur / arr.length, 0);

console.log(calcDogAge([5, 2, 4, 1, 15, 8, 3]));
console.log(calcDogAge([16, 6, 10, 5, 6, 1, 4]));

console.log(calcDogAge2([5, 2, 4, 1, 15, 8, 3]));
console.log(calcDogAge2([16, 6, 10, 5, 6, 1, 4]));
*/
...
//     dog.eating = 'EatTooMuch';
//   } else if (dog.curFood < dog.recommended * 0.9) {
//     dog.eating = 'EatTooLittle';
//   } else {
//     dog.eating = 'Good Amount';
//   }
// });
